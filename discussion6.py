...
###############################################################################
    ...
    def horse_fastest_race(self, horse):
        ...
        if horse in self.race_dict:
            times = self.race_dict[horse]
            fastest_time = min(times, key = times.get)
            result = times[fastest_time]
            return (fastest_time, result)
        else:
            return(None, 999.9)
...
###############################################################################
    ...
    def horse_personal_best(self):
        ...
        for horse in self.race_dict:
            d[horse] = self.horse_fastest_race(horse)
        return d
